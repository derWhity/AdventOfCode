...
func (b *board) String() string {
	...
	for _, row := range b.entries {
		for _, col := range row {
			...
			if col.marked {
				if b.won {
					val = aurora.BrightRed(fmt.Sprintf("%2d ", col.num)).String()
				} else {
...
func main() {
	...
	for round, numStr := range drawnNumbers {
		...
		for _, b := range boards {
			bCopy := *b
			if b.numberDrawn(num) {
				numWon++
				lastBoard = &bCopy
			}
			fmt.Println(b.String())
			if numWon == numBoards {
				score := lastBoard.getSumOfUnmarkedNumbers() * num
				fmt.Printf("Bingo!!! - Last Board has %s points\n", aurora.Bold(aurora.BrightRed(fmt.Sprintf("%d", score))))
				os.Exit(0)
			}
		}
	}
